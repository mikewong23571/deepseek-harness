...
export const Config: z<ConnectionConfig> = z.object({
  ...
  allowTrustedHostConfiguration: z.boolean().default(false),
  maxRequestBodyBytes: z.natural().min(1).default(DEFAULT_MAX_REQUEST_BODY_BYTES),
})

/** Methods always gated to loopback, even when trusted-host configuration is enabled. */
...
/** Model configuration methods optionally available to trusted non-loopback authorities. */
const CONFIGURATION_METHODS = new Set([
  'settings.describe',
  'settings.update',
  'settings.replace',
  'settings.mutate',
 ...
 * host-native methods always pass it with an empty trust list. Model
 * configuration methods do the same unless the deployment explicitly enables
...
export function apply(ctx: Context, config?: ConnectionConfig): void {
  ...
  const allowTrustedHostConfiguration = config?.allowTrustedHostConfiguration ?? false
  ...
  const fetchHandler = connection.createSharedFetchHandler(API_PATH, {
    async fetch(request) {
      ...
      const method = pathname.startsWith(`${API_PATH}/`)
        ...
        : undefined
      const loopbackOnly = method !== undefined
        && (LOOPBACK_ONLY_METHODS.has(method)
          || (!allowTrustedHostConfiguration && CONFIGURATION_METHODS.has(method)))
      if (loopbackOnly && !isTrustedApiRequest(request, [])) {
