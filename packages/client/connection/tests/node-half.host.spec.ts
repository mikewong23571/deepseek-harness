...
async function mounted(config?: { trustedHosts?: string[]; allowTrustedHostConfiguration?: boolean }): Promise<{
  routes: WebRoute[]
  upgrades: WebUpgradeRoute[]
...
describe('connection node half', () => {
  ...
  it('keeps configuration loopback-only by default for a declared trusted authority', async () => {
  ...
  it('opens configuration explicitly while host-native methods remain loopback-only', async () => {
    const { routes, dispose } = await mounted({
      trustedHosts: ['harness.example'],
      allowTrustedHostConfiguration: true,
    })
    for (const method of [
      'settings.describe', 'settings.update', 'settings.replace', 'settings.mutate',
      'credentials.describe', 'credentials.set', 'credentials.unset', 'llm.discoverModels',
      ...
      await routes[0]!.handler(fakeRequest({ host: 'harness.example' }, `${API_PATH}/${method}`), passed.response)
...
describe('connection node half over a real HTTP server', () => {
  ...
  it('opens trusted-host configuration explicitly over real HTTP', async () => {
    const { routes, dispose } = await mounted({
      trustedHosts: ['harness.example'],
      allowTrustedHostConfiguration: true,
    })
