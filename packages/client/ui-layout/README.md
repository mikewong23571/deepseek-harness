...
AppFrame always mounts the conversation and details columns; a connected Session renders through `SessionProvider`. The transient layout store starts the sidebar at its default width and details closed, and it never reads or writes `localStorage`. Hero and other unselected states also derive a zero rendered details width without changing that stored preference. AppFrame retains the last non-blank Session id across those states: the first Session remains closed, an explicit details action opens the contract default width, returning to the same Session restores its unchanged width, and selecting a different Session closes details before paint. The conversation owner share is empty, while the sidebar owner share carries `collapsed`, `width`, and `picker`; registrants obtain business data from standard hooks and actions from their own inject faces. Below the 1024px auto-collapse breakpoint the frame switches to the mobile tree: a full-width top header (the current-session switcher and a settings gear signaled through `ctx.layout`), the conversation at full width, and the sidebar slot mounted as a dropdown picker (`picker: true`) under the header — the same narrowExpanded override the rail toggle used, closed by a mask tap or a session pick.
