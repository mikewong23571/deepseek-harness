...
`)
  writeFileSync(join(dir, 'cordis.yml'), [
    ...
    '    allowTrustedHostConfiguration: !!js ctx.webStartup.allowTrustedHostConfiguration',
    '- id: provider',
...
describe('web command-line provider', () => {
  ...
  it('leaves deployment values to each consumer when flags omit them', async () => {
    const { values, observed } = await bootProvider([])
    expect(values).toEqual({ trustedHosts: [], allowTrustedHostConfiguration: false })
    expect(observed.readerConfig).toEqual({
      host: '127.0.0.1',
      port: 3080,
      trustedHosts: [],
      allowTrustedHostConfiguration: false,
    })
  })
  ...
  it('prints its own help and leaves the consumer pending', async () => {
    ...
    expect(observed.out).toContain('--allow-trusted-host-configuration')
