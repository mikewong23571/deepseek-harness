...
Two boundaries stay deliberately out of scope: reachability is the webserver binding's policy (a required hostname or address), and authentication for genuinely remote deployments is deferred work recorded in the connection README — the fence is a confused-deputy defense, not an auth layer. The old guard's loopback-socket check was dropped rather than generalized: with binding expressing reachability and `trustedHosts` naming remote authorities, the socket address adds nothing a header fence does not already cover.
...
- Non-loopback deployments must have their serving authorities trusted or requests are refused. The dsh CLI derives a specific bind address into the connection row and offers `dsh web --trusted-host` for additional named authorities; all-interface programmatic compositions derive current LAN IP literals, while other compositions declare `trustedHosts` themselves. Non-browser automation rides the same fence: loopback, a bind-derived IP, or a declared authority passes; an undeclared DNS alias is refused.
- Clients must label POST bodies `application/json` (ours always did; raw-fetch tests gained the header).
- Any unauthenticated non-loopback deployment assumes the bound network is trusted.
